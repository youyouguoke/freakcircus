...
const locations = [
  ...
  },
  {
    ...
    significance: "Referenced in Day 2 backstory — the Columbina legend",
  ...
  },
  {
    ...
    significance: "The Doctor's workspace — contains medical files and character profiles",
    secrets: "1920s medical equipment, Project Continuity files, window shows different exteriors",
  },
  {
    name: "Basement Stage",
    status: "Corrupted",
    desc: "The Basement Stage is a location referenced in community reports. Whether it exists in the current build is unverified. Some glitch states. It is described as a performance space beneath the main tent, but its architecture suggests it was built before the tent above it. The stage contains props from performances that never occurred in the game's timeline, including posters for shows with dates that do not exist. The Basement's lighting system flickers in patterns that match Morse code, spelling out phrases like 'NO ONE IS HERE' and 'YOU ARE ALONE'.",
    significance: "Referenced in community reports — existence in current build is unverified",
    secrets: "Props from non-existent performances, Morse code lighting, predates main tent",
  },
  {
    name: "Silent Exit",
    status: "Hidden",
    desc: "The Silent Exit is a door referenced in community reports. Whether it exists in the current build is unverified. It is described as leading 'outside the circus' but shows only darkness when opened. Some players report hearing their own voice when approaching the Exit, and the door handle matches the player's mouse cursor. The Exit's frame contains carvings that match symbols from the Mirror Hall, suggesting a connection between these two hidden locations. Opening the Exit causes the game to crash on some systems, and data miners found an unused scene showing what is behind it.",
    significance: "Referenced in community reports — existence in current build is unverified",
